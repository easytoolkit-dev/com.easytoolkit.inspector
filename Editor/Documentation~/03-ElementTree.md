...
public interface IElementTree
{
    ...
    ElementCreator ElementCreator { get; }   // 元素创建器
    VisualElement RootVisualElement { get; } // UI Toolkit 根节点
    InspectorBackendMode BackendMode { get; } // 渲染后端
}
```
...
**位置**: [Core/Elements/Implementations/ElementTree.cs](../Core/Elements/Implementations/ElementTree.cs)
...
## 元素创建器 (ElementCreator)

**位置**: [Core/Elements/Models/ElementCreator.cs](../Core/Elements/Models/ElementCreator.cs)

### 职责

元素创建器由 ElementTree 持有，负责创建、登记和销毁各种类型的元素：

```csharp
public class ElementCreator
{
    IElement CreateElement(IElementDefinition definition, ILogicalElement parent);
    ...
    bool DestroyElement(IElement element);
}
```
...
IElementDefinition
    ↓
ElementCreator.CreateElement(definition, parent)
    ↓
...
## 元素树工厂 (ElementTreeFactory)

**位置**: [Core/Factories/ElementTreeFactory.cs](../Core/Factories/ElementTreeFactory.cs)
...
{
    IElementTree CreateTree(SerializedObject serializedObject, InspectorBackendMode backendMode = InspectorBackendMode.IMGUI);
    IElementTree CreateTree(object[] targets, SerializedObject serializedObject, InspectorBackendMode backendMode = InspectorBackendMode.IMGUI);
}
```
...
`ElementTreeFactory` 是当前创建元素树的统一入口。元素定义由结构解析器直接创建，元素实例由树内的 `ElementCreator` 创建。

### 使用示例

```csharp
// 创建元素树
var tree = ElementTreeFactory.CreateTree(serializedObject);

// 创建元素
var element = tree.ElementCreator.CreateElement(definition, parent);
```
...
3. **事件分发** - 注册、批处理并触发元素事件

### 相关文件

**位置**: [Core/Elements/Models/ElementSharedContext.cs](../Core/Elements/Models/ElementSharedContext.cs)
