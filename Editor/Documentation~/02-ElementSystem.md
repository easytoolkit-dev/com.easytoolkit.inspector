...
public interface IElement
{
    ...
    ElementSharedContext SharedContext { get; }         // 共享上下文（树级服务）
    IElement Parent { get; }                            // 当前父元素（动态）
    ElementState State { get; }                         // 元素运行时状态
    string Path { get; }                                // 层级路径
    GUIContent Label { get; set; }                      // 显示标签
    ElementPhases Phases { get; }                       // 当前生命周期阶段
    ElementList<IElement> Children { get; }             // 子元素集合
    VisualElement VisualElement { get; }                // UI Toolkit 视觉节点
}
```
...
- **结构解析器基类**: [UI/Resolvers/StructureResolver/ValueStructureResolverBase.cs](../UI/Resolvers/StructureResolver/ValueStructureResolverBase.cs)
- **集合结构解析器**: [UI/Resolvers/StructureResolver/Collection/CollectionStructureResolver.cs](../UI/Resolvers/StructureResolver/Collection/CollectionStructureResolver.cs)
...
public interface IValueElement : ILogicalElement
{
    ...
    // 运行时动态添加的子元素
    ElementList<IElement> Children { get; }
...
- **实现**: [Core/Elements/Implementations/ValueElement.cs](../Core/Elements/Implementations/ValueElement.cs)
- **值操作解析器**: [Core/Resolvers/IValueOperationResolver.cs](../Core/Resolvers/IValueOperationResolver.cs)
...
- **方法结构解析器**: [UI/Resolvers/StructureResolver/MethodStructureResolverBase.cs](../UI/Resolvers/StructureResolver/MethodStructureResolverBase.cs)
...
- **后处理器**: [UI/PostProcessors/GroupElementPostProcessor.cs](../UI/PostProcessors/GroupElementPostProcessor.cs)
...
- **定义**: [Core/Definitions/Abstractions/IRootDefinition.cs](../Core/Definitions/Abstractions/IRootDefinition.cs)
...
- **消息分发器**: [Infrastructure/Messaging/MessageDispatcherFactory.cs](../Infrastructure/Messaging/MessageDispatcherFactory.cs)

## 定义系统

### IElementDefinition - 元素定义

不可变定义对象包含元素的元数据，解析器根据定义创建字段、属性、集合项、方法参数和分组等元素。

**相关文件**:
- **定义接口**: [Core/Definitions/Abstractions/](../Core/Definitions/Abstractions/)
- **定义实现**: [Core/Definitions/Implementations/](../Core/Definitions/Implementations/)
...
**位置**: [Core/Elements/Implementations/ValueElement.cs](../Core/Elements/Implementations/ValueElement.cs)
...
**位置**:
- [Core/Elements/Implementations/PropertyElement.cs](../Core/Elements/Implementations/PropertyElement.cs)
- [Core/Elements/Implementations/FieldElement.cs](../Core/Elements/Implementations/FieldElement.cs)
