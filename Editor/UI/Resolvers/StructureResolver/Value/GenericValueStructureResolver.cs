...
namespace EasyToolkit.Inspector.Editor
{
    ...
    public class GenericValueStructureResolver : ValueStructureResolverBase
    {
        ...
        private IElementDefinition CreateDefinition(MemberInfo memberInfo)
        {
            ...
            {
                return new MethodDefinition(methodInfo);
            }
            ...
            if (memberInfo is FieldInfo fieldInfo)
            {
                ...
                return new FieldDefinition(fieldInfo);
            }
            ...
            if (memberInfo is PropertyInfo propertyInfo)
            {
                ...
                return new PropertyDefinition(propertyInfo);
            }
        ...
        private bool TryCreateCollectionDefinition(MemberInfo memberInfo, out ICollectionDefinition collectionDefinition)
        {
            ...
            if (memberInfo is FieldInfo fieldInfo)
            {
                collectionDefinition = new FieldCollectionDefinition(fieldInfo, elementType, isOrdered);
            }
            else if (memberInfo is PropertyInfo propertyInfo)
            {
                collectionDefinition = new PropertyCollectionDefinition(propertyInfo, elementType, isOrdered);
            }
            else
            {
                return false;
